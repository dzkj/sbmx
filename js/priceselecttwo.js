	...
	$(document).ready(function(){
		//初始场次第一个选中 BY lsy
			...
			$("#Jprice li").filter('[w=1]').not('[class=over]').mouseenter(function(){
				...
				$('.js-mobile').css({left:iLeft,top:iTop-206,display:'block'});
...
//场次点击选择    
var cqTime = $('#Jdate li:first').attr('title');;    //场次时间
var dz_num = 0;
...
$('.price-ck li').click(function(){
	...
	if(_this.attr('class') != 'over' && _this.attr('class') != 'over has_no_product'){    //非卖完状态  -----或者通过pstatus来判断-----
		...
		var status = $(this).attr('s'), ppid = $(this).attr('p'), n = $(this).attr('n'), num = $(this).attr('l');
...
//	    	var oldFlashUrl = $("#Jdate").children(".on").attr("cc");
	    	...
	    	var changeUrl = getPath()+"/ajax/highLightPrice";
	    	$.getJSON(changeUrl, {oldFlashUrl:oldFlashUrl,xzPriceId:xzPriceId} ,function(data){
	    		if (data != '') { 
	    			seatUrl=xzurl + ((xzurl.indexOf("?") != -1) ? '&' : '?');
	    			//$('.zxxz').attr('href', data + ((data.indexOf("?") != -1) ? '&' : '?')).show();
	    			$('.zxxz').attr('href', 'javascript:getSeatFunction();');
				} else { 
					seatUrl=null;
					$('.zxxz').attr('href', 'javascript:void(0);').hide();
...
//			});
			...
			var changeUrlCl = getPath()+"/ajax/highLightCanel";
	    	$.getJSON(changeUrlCl, {oldFlashUrlCl:oldFlashUrlCl,priceid:priceid} ,function(data){
	    		//$('.zxxz').attr('href', data + ((data.indexOf("?") != -1) ? '&' : '?')).show();
	    		seatUrl=xzurl + ((xzurl.indexOf("?") != -1) ? '&' : '?');
	    		$('.zxxz').attr('href', 'javascript:getSeatFunction();');
	    	});
...
$('.price-ck .over').hover(function(){
    ...
    $(this).css({'background-position':'100% -98px'}).find('i').text('缺货 碰碰运气~');
},function(){
	//项目票状态为"已售完"
//	if(productStatus=='2'){
	if(productStatus=='4'){
//		if(productplaystatus=='2' || productNum == '0'){
		$(this).css({'background-position':'100% -54px'}).find('i').text(priceInfo);
	}
	else{
	    $(this).css({'background-position':'100% -205px'}).find('i').text(priceInfo);
	}
});
//ie6下票价框滑动红色框修复
if(ie6){
...
$('.btn-now-buy').click(function(){
    ...
    if (null == params || '^' == params) {
        ...
        // ajax 校验库存
        $.post(getPath() + '/ajax/checkPayLimit', {sd: params }, function(d){
            if (d.status == 0) {
                var qzparam = isqz == '1' ? "&isqz=" + isqz : ""; 
              //人鬼情未了活动
                if(productid == 63046717){
                	document.location.href = getPath() + '/cart/quickbuy?pid=' + pid + '&sd=' + params + qzparam + '&ghostCode=' + ghostCode;
                }else if(productid == 63375370){
                	//高尔夫深圳国际赛活动
                	document.location.href = getPath() + '/cart/quickbuy?pid=' + pid + '&sd=' + params + qzparam + '&golfCode=' + golfCode;
                }else if(pids.join(',').indexOf(productid)>=0){
                	//LP活动
                	document.location.href = getPath() + '/cart/quickbuy?pid=' + pid + '&sd=' + params + qzparam + '&lpCode=' + lpCode;
                }else{
                	//遮罩提交开始
                	...
                	if(isLimit == "1" && customerFlag =="1"){
                    		var url = getPath()+"/cart/CusLimitTic";
                     		 $.post(url,{productid:productid},function(date){
                     			 if(!date){
                     				alert_sussnew("不能购买!", "已超过商品限购订单数，不能继续购买。");	
                     				return false;
                     			 }else{
                             			document.location.href = getPath_fix() + '/cart/quickbuy?pid=' + pid + '&sd=' + params + qzparam;	
                     			 }
                   	 	 });		
                	}else{
                		document.location.href = getPath_fix() + '/cart/quickbuy?pid=' + pid + '&sd=' + params + qzparam;	
...
$('.dianzi').click(function(){
    ...
    if (null == params || '^' == params) {
        ...
        document.location.href = '/dzcomplete.html?pid=' + pid + '&sd=' + params;
    }
    return false;
});
...
function getSeatFunction(){
	$.get(getPath() + '/ajax/getSeatToken',function(data){
		if(data == 'yes'){
			document.location.href = seatUrl;
		}else{
