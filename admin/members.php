 ...
 <div class="rt_content">
     ...
     $(document).ready(function(){
		 ...
		 $(".trueBtn").click(function(){			 
			 var id = $("#del_id").val();
			 location.href="members_action.php?id="+id;
			 $(".pop_bg").fadeOut(0.1);
			 });
		 //弹出：取消或关闭按钮
      ...
      <div class="pop_cont">
       ...
       <div class="btm_btn">
		...
		if (isset($_GET['page']))   
			...
			$page=1;
		$gres=mysql_query("select count(*) from members"); //获得记录总数
		...
		$result=mysql_query("select * from members order by id desc limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数

		?>
     <section>
      <div class="page_title">
       <h2 class="fl">会员信息列表</h2>
		...
		<ul style="margin-top:5px;background: #19a97b;" class="fr top_rt_btn" id="nav">
			...
			<table class="table">
			   <tr>
					...
					<th>邮箱</th>
					<th>会员名<br>
					<th>密码<br><!--<font style="color: #F7460E;">(悬浮查看详情)</font></th>-->
					<th>昵称</th>
					<th>性别</th>
					<th>出生日期</th>
					<th>身份证号</th>
			...
			?>
					<tr>
                        ...
                        <td><?php echo $row['ident_num'] ;?></td>
						 <td><?php echo $row['city_location'] ;?></td>
                        <td><?php echo $row['marr_status'] ;?></td>
						    <td><?php echo $row['chil_status'] ;?></td>
                        <td><?php echo $row['engage_work']; ?></td>
                        <td><?php echo $row['position_work'] ;?></td>
                        <td><?php echo $row['edu_level'] ;?></td>
                        <td><?php echo $row['Income_level'] ;?></td>
						<td style="min-width:180px;width:200px;">
						 <a href="members_edit.php?id=<?php echo $row['id'] ?>" class="inner_btne">重置密码</a>
