...
if(isset($_POST['csubmit'])){
	...
	mysql_query("update memcredits set status='1',credits='$credits',bank='$bank',redstatus='0',backinfo='$backinfo' where id='$cre_id'");
	mysql_query("update member set credits='$jifen' where id='$bid'");
	
	if(mysql_affected_rows()>=0){
		echo "<script>alert('提现成功!');</script>";
		echo "<script>location.href='member_wtx.php'</script>";
	}else{
