      ...
      <div class="pop_cont">
       ...
       <div class="btm_btn">
		...
		if (isset($_GET['page']))   
			...
			$page=1;
		$gres=mysql_query("select count(*) from memcredits where status='0' and actionname='充值'"); //获得记录总数
		...
		$result=mysql_query("select a.bydate,a.credits as acredits,a.id as aid,b.* from memcredits as a,member as b where a.mid=b.id and a.status='0' and a.actionname='充值' order by bydate desc limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
     ...
     <section>
      ...
      <table class="table">
	   ...
	   <?php
		...
		?>
		<form action="member_wcz_do.php" method="post" enctype="multipart/form-data">
		   <tr>
			<input type="hidden" value="<?php echo $prow['aid'];?>" name="cre_id"/>
			<td><div style="width:200px;cursor:pointer" title="电话:<?php echo $prow['tel'];?>&nbsp;银行卡:<?php echo $prow['bank'];?>" class="cut_title ellipsis"><?php echo $prow['name'];?></div></td>
			...
			<td style="min-width:180px;width:200px;">
			 <input name="csubmit" style="border:0px;" type="submit" class="inner_btne" value="确定充值"/>
