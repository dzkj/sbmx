...
if (isset($_GET['where'])) {
    $a = $_GET['where'];
    $where = "where ask_quest like '%" . $_GET['where'] . "%'";
} else {
    $a = "";
    $where = "";
}
        ...
        <section class="rt_wrap content mCustomScrollbar">
            <div class="rt_content">
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        $(".trueBtn").click(function() {
                            var id = $("#del_id").val();
                            location.href = "faqs_action.php?del=yes&id=" + id;
                            $(".pop_bg").fadeOut(0.1);
                        });
                        //弹出：取消或关闭按钮
                        ...
                        $("#find").click(function() {
                            ...
                            //alert(where);
                            location.href = "faqs.php?where=" + where;
                        });
                    });
                ...
                if (isset($_GET['page']))
                    ...
                    $page = 1;
                $gres = mysql_query("select count(*) from faqs " . $where); //获得记录总数
                ...
                $result = mysql_query("select * from faqs " . $where . " order by state limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
                ?>
                <section>
                    <div class="page_title">
                        <h2 class="fl">问答列表</h2>
                        <input style="margin-left:15px;height:18px;" type="text" id="likefind" value="<?php echo $a; ?>" class="textbox textbox_225" placeholder="请输入问题名"/>
                    ...
                    <table class="table">
                        ...
                        while ($row = mysql_fetch_array($result)) {
                            ?>
                            <tr>
                                <td><div style="width:200px;" class=" ellipsis"><?php echo $row['ask_quest'];
                                        ?></div></td>
                                <td><?php echo $row['ask_name'] ;?></td>
                                <td><?php echo $row['ask_date']; ?></td>
								<td><?php if($row['state']==1){ echo "已回复";}else{ echo "未回复";} ?></td>
                                <td style="min-width:180px;width:200px;">
								<?php if($row['state']!=1){ ?>
                                    <a href="faqs_edit.php?id=<?php echo $row['id']; ?>" class="inner_btne">回复</a>
								<?php
									}else{
								?>
								    <a href="faqs_examine.php?id=<?php echo $row['id']; ?>" class="inner_btne">查看</a>
								<?php
									}
								?>
